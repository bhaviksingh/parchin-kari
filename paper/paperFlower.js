...
class PaperPetal {
  ...
  drawLeaf() {
    ...
    for (let i = 0; i <= subdivs; i++) {
      ...
      let flangle =
        i >= this.curveIndex ? petalNode.angle + (i - this.curveIndex + 1) * this.curveIncrement : petalNode.angle;
...
class PaperFlower {
  constructor(node) {
    ...
    this.pistilSize = node.size * (node.pistilSize || 0.25);
    this.numPetals = node.numPetals || 5;
    this.petalArcAngle = node.petalArcAngle || Math.PI;
    this.pistilColor = node.pistilColor || "yellow";
  ...
  drawFlower() {
    ...
    let leafWidth = this.flowerNode.width || (this.pistilSize * this.petalArcAngle * 1.5) / this.numPetals;

    if (false) {
      let debugCircle = new paper.Path.Circle({
        x: this.flowerNode.x,
        y: this.flowerNode.y,
        radius: this.flowerNode.size,
        strokeColor: "black",
      });
      this.myGroup.addChild(debugCircle);
    }
    ...
    for (let i = -numPerSide; i <= numPerSide; i++) {
      ...
      let petalCoords = nextXY({ ...this.flowerNode, angle: petalAngle },  0.6 * this.pistilSize);
      let petal = new PaperPetal({
        ...this.flowerNode,
        x: petalCoords.x,
        y: petalCoords.y,
        angle: petalAngle,
        size: this.flowerNode.size - this.pistilSize,
        width: leafWidth,
      });
  ...
  drawPistil() {
    if (this.showPistil) {
      ...
      this.pistil.fillColor = this.pistilColor;
